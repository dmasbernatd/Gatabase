"""El campo del Tutor que normaliza lo que se escribe: el RUT.
...
El campo de formulario que devuelve hace lo mismo al entrar —así el propio
formulario puede preguntar por duplicados con el valor ya normalizado, antes de
guardar— y al salir lo presenta como se lee.

El del teléfono estaba aquí y se fue a `apps/campos.py` cuando la Sede que
atiende urgencias necesitó uno: `tenancy` no puede importar de `tutors`. Se
sigue nombrando aquí porque la migración `0003` lo llama por este camino, y una
migración ya aplicada no se reescribe.
"""

from django import forms

from apps.campos import CampoDeTelefono, CampoQueNormaliza  # noqa: F401
