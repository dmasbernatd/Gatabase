...
- **En el listado de Tutores no se busca a cada tecla.** Se dispara al enviar la búsqueda y al pulsar una cabecera o una página. La caja del mostrador sí busca mientras se escribe, y lo que la hace admisible es dónde cae la línea: **una búsqueda anota que se consultó el conjunto; abrir una ficha anota a esa persona.** Lo que llenaría de ruido la tabla que tiene que valer como prueba es lo segundo hecho por lo primero — veinte nombres anotados por pasar por delante mientras alguien teclea —, no la constancia de que se buscó. Sin ninguna constancia, una pantalla que enseña veinte nombres y veinte teléfonos no dejaría rastro, y eso es lo que ADR-0004 no admite. La caja espera 250 ms desde la última tecla, así que un nombre escrito de corrido es una petición, no una por letra.
